...
#define MAGIC_VALUE nullptr //INT_MAX для стэка с интовскими элементами

typedef TreeNode** stack_data; //NOTE: для данного стэка мы поменяли с double* на int*
#define STACK_PRINTF_SPEC "%d" //УДОБНО ДЛЯ ТОГО, ЕСЛИ МЫ РЕШИМ ГЛОБАЛЬНО ПОМЕНЯТЬ С double на int,
//ТО У НАС ПОМЕНЯЮТСЯ И СПЕЦИФИКАТОРЫ

typedef TreeNode* stackElem_t;
